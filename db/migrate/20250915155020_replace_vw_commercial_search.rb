class ReplaceVwCommercialSearch < ActiveRecord::Migration[7.0]
  def self.sql
    <<~SQL
      SELECT
  ...
  def self.up
    ...
    execute "DROP VIEW IF EXISTS vw_commercial_search"

    execute "CREATE OR REPLACE VIEW vw_commercial_yesterday AS
