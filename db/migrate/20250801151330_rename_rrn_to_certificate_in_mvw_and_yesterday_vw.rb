class RenameRrnToCertificateInMvwAndYesterdayVw < ActiveRecord::Migration[7.0]
  def self.sql
    <<~SQL
      SELECT
      ...
      (REPLACE(get_attribute_value('assessment_address_id'::character varying, ad.assessment_id),  'UPRN-', '')::BIGINT)::varchar(15)
       ELSE '' END as building_reference_number,
        ...
        get_lookup_value('construction_age_band', get_attribute_json('sap_building_parts', ad.assessment_id) -> 0 ->> 'construction_age_band', ad.document ->> 'assessment_type', get_attribute_value('schema_type', ad.assessment_id)) as construction_age_band,
        ...
        get_lookup_value('energy_efficiency_rating', COALESCE(get_attribute_json('window', ad.assessment_id) ->> 'energy_efficiency_rating', get_attribute_json('windows', ad.assessment_id) ->> 'energy_efficiency_rating'), ad.document ->> 'assessment_type', get_attribute_value('schema_type', ad.assessment_id)) as windows_energy_eff,
        get_lookup_value('energy_efficiency_rating', COALESCE(get_attribute_json('window', ad.assessment_id) ->> 'environmental_efficiency_rating', get_attribute_json('windows', ad.assessment_id) ->> 'environmental_efficiency_rating'), ad.document ->> 'assessment_type', get_attribute_value('schema_type', ad.assessment_id)) as windows_env_eff,
        ...
        COALESCE(
          get_attribute_value('incandescent_fixed_lighting_bulbs_count', ad.assessment_id),
  ...
  def self.sql_rr
    <<-SQL
    SELECT
           aav.assessment_id as certificate_number,
  ...
  def self.up
    ...
    execute "CREATE MATERIALIZED VIEW mvw_domestic_rr_search as #{sql_rr} WITH NO DATA;"

    execute "DROP VIEW IF EXISTS vw_domestic_rr_yesterday"
    execute "CREATE VIEW vw_domestic_rr_yesterday as #{sql_rr} AND ad.warehouse_created_at::date = CURRENT_DATE - 1;;"
  end
