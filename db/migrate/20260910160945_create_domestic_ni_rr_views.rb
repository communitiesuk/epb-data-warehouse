class CreateDomesticNiRrViews < ActiveRecord::Migration[8.1]
  def self.sql
    <<~SQL
          select
  ...
  def self.up
    execute "DROP MATERIALIZED VIEW IF EXISTS mvw_domestic_ni_rr_search"
    execute "CREATE MATERIALIZED VIEW mvw_domestic_ni_rr_search AS #{sql} WITH NO DATA;"

    execute "DROP VIEW IF EXISTS vw_domestic_ni_rr_yesterday"
    execute "CREATE VIEW vw_domestic_ni_rr_yesterday AS #{sql} AND ((ad.warehouse_created_at)::date = (CURRENT_DATE - 1));"
  end
end
