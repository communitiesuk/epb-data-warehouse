class UseAssessmentSearchTableToFilterDataOnDomesticMvwAndYesterdayVw < ActiveRecord::Migration[7.0]
  def self.sql
    <<~SQL
      SELECT
      ...
      JOIN ( VALUES ('SAP'), ('RdSAP') ) vals (t) ON (assessment_type = t)
  ...
  def self.up
    ...
    AS #{sql} WHERE warehouse_created_at::date = CURRENT_DATE - 1 OR updated_at::date = (CURRENT_DATE - 1)"
  end
