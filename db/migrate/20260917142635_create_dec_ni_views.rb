class CreateDecNiViews < ActiveRecord::Migration[8.1]
  def self.sql
    <<~SQL
      SELECT
  ...
  def self.up
    ...
    execute "DROP MATERIALIZED VIEW IF EXISTS mvw_dec_ni_search"
    execute "CREATE MATERIALIZED VIEW mvw_dec_ni_search AS
    #{sql} WITH NO DATA;"

    execute "DROP VIEW IF EXISTS vw_dec_ni_yesterday"
    execute "CREATE VIEW vw_dec_ni_yesterday AS
    #{sql} #{yesterday_condition}"

    execute "DROP MATERIALIZED VIEW IF EXISTS mvw_dec_ni_rr_search"
    execute "CREATE MATERIALIZED VIEW mvw_dec_ni_rr_search AS
    #{sql_rr} WITH NO DATA;"

    execute "DROP VIEW IF EXISTS vw_dec_ni_rr_yesterday"
    execute "CREATE OR REPLACE VIEW vw_dec_ni_rr_yesterday AS #{sql_rr(additional_filter: 'AND s.created_at::date = (CURRENT_DATE - 1)')}"
  end
end
