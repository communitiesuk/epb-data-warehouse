class ExcludeSap15FromMvwDomesticSearch < ActiveRecord::Migration[7.0]
  def self.sql
    <<~SQL
      SELECT
      ...
      WHERE get_attribute_value('schema_type', ad.assessment_id) NOT LIKE 'SAP-Schema-15%'
    SQL
  end
  ...
  def self.up
    ...
    #{sql} AND s.created_at::date = (CURRENT_DATE - 1) OR updated_at::date = (CURRENT_DATE - 1)"
  end
