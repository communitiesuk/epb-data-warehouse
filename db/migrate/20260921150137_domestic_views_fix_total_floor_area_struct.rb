class DomesticViewsFixTotalFloorAreaStruct < ActiveRecord::Migration[8.1]
  def self.sql(country_codes = [])
    query = <<~SQL
      SELECT
        ...
        COALESCE(
                    ad.document -> 'co2_emissions_current' ->> 'value',
                    ad.document ->> 'co2_emissions_current'
                ) AS CO2_EMISSIONS_CURRENT,
                COALESCE(
                    ad.document -> 'co2_emissions_current_per_floor_area' ->> 'value',
                    ad.document ->> 'co2_emissions_current_per_floor_area'
                ) AS CO2_EMISS_CURR_PER_FLOOR_AREA,
                COALESCE(
                    ad.document -> 'co2_emissions_potential' ->> 'value',
                    ad.document ->> 'co2_emissions_potential'
                ) AS CO2_EMISSIONS_POTENTIAL,
        COALESCE(
          ad.document ->> 'total_floor_area',
          (
            SELECT ROUND(SUM(val))::text
            FROM (
              SELECT COALESCE(
                (area ->> 'value')::numeric,#{' '}
                (area #>> '{}')::numeric
              ...
              FROM jsonb_array_elements(
                jsonb_path_query_array(ad.document, '$.sap_building_parts[*].sap_floor_dimensions[*].total_floor_area')
              ) area
              UNION ALL
              SELECT COALESCE(
                (area ->> 'value')::numeric,#{' '}
              ...
              FROM jsonb_array_elements(
                jsonb_path_query_array(ad.document, '$.sap_building_parts[*].sap_room_in_roof.floor_area')
              ) area
            ) all_areas
          )
        ) AS TOTAL_FLOOR_AREA,
        ...
        COALESCE(
            get_lookup_value('block_storey', ad.document -> 'sap_flat_details' ->> 'flat_location', s.assessment_type, ad.document ->> 'schema_type'),
            get_lookup_value('flat_level', ad.document -> 'sap_flat_details' ->> 'level', s.assessment_type, ad.document ->> 'schema_type')
            ) AS FLOOR_LEVEL,
    ...
    if country_codes.any?
      codes = country_codes.map { |c| "'#{c}'" }.join(", ")
      query += " WHERE co.country_code IN (#{codes})"
    end
  ...
  def self.up
    yesterday_condition = " AND (s.created_at::date = (CURRENT_DATE - 1) OR EXISTS(SELECT * FROM audit_logs l WHERE s.assessment_id=l.assessment_id AND event_type = 'address_id_updated' AND timestamp::date = (CURRENT_DATE - 1)))"

    execute "DROP MATERIALIZED VIEW IF EXISTS mvw_domestic_search"
    execute "CREATE MATERIALIZED VIEW mvw_domestic_search AS
             #{sql(%w[EAW ENG WLS])} WITH NO DATA;"

    execute "DROP VIEW IF EXISTS vw_domestic_yesterday"
    execute "CREATE VIEW vw_domestic_yesterday AS
             #{sql(%w[EAW ENG WLS])} #{yesterday_condition}"

    execute "DROP MATERIALIZED VIEW IF EXISTS mvw_domestic_ni_search"
    execute "CREATE MATERIALIZED VIEW mvw_domestic_ni_search AS
             #{sql(%w[NIR])} WITH NO DATA;"

    execute "DROP VIEW IF EXISTS vw_domestic_ni_yesterday"
    execute "CREATE VIEW vw_domestic_ni_yesterday AS
             #{sql(%w[NIR])} #{yesterday_condition}"
  end
