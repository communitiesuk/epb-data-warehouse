class UpdateCommercialMvwYesterdayAndRecommendationsToFilterNiData < ActiveRecord::Migration[8.1]
  def self.sql
    <<~SQL
      SELECT
  ...
  AS #{sql} WITH NO DATA;"
    ...
    execute "CREATE MATERIALIZED VIEW mvw_commercial_rr_search AS #{sql_rr} WITH NO DATA;"

    execute "DROP VIEW IF EXISTS vw_commercial_rr_yesterday"
    execute "CREATE OR REPLACE VIEW vw_commercial_rr_yesterday AS #{sql_rr(additional_filter: 'AND s.created_at::date = (CURRENT_DATE - 1)')}"
  end
end
