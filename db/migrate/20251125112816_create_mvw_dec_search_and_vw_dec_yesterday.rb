class CreateMvwDecSearchAndVwDecYesterday < ActiveRecord::Migration[7.0]
  def self.sql
    <<~SQL
      SELECT
        ...
        get_attribute_json('technical_information',  ad.assessment_id) ->> 'floor_area' AS TOTAL_FLOOR_AREA,
        get_attribute_json('or_benchmark_data',  ad.assessment_id) -> 'benchmarks' -> 0 ->> 'occupancy_level' AS OCCUPANCY_LEVEL,
        get_attribute_json('dec_annual_energy_summary',  ad.assessment_id) ->> 'annual_energy_use_fuel_thermal' AS ANNUAL_THERMAL_FUEL_USAGE,
        get_attribute_json('dec_annual_energy_summary',  ad.assessment_id) ->> 'typical_thermal_use' AS TYPICAL_THERMAL_FUEL_USAGE,
        get_attribute_json('dec_annual_energy_summary',  ad.assessment_id) ->> 'annual_energy_use_electrical' AS ANNUAL_ELECTRICAL_FUEL_USAGE,
        ...
        get_attribute_json('dec_annual_energy_summary',  ad.assessment_id) ->> 'typical_thermal_use' AS TYPICAL_THERMAL_USE,
        get_attribute_value('report_type', ad.assessment_id) AS REPORT_TYPE,
        get_attribute_value('reason_type', ad.assessment_id) AS REASON_TYPE,
