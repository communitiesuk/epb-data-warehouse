class AlterMaterilizedViewAvgCo2V2 < ActiveRecord::Migration[7.0]
  def self.up
    execute "DROP MATERIALIZED VIEW IF EXISTS mvw_avg_co2_emissions"
    execute "CREATE SEQUENCE IF NOT EXISTS avg_co2_emissions_seq INCREMENT 1 START 1;"
    ...
    execute <<~SQL
      CREATE MATERIALIZED VIEW mvw_avg_co2_emissions
           ...
           where (document ->> 'assessment_type')::varchar IN ('SAP', 'RdSAP')
           and (ad.document ->> 'registration_date') >= '2020-10-01'
           group by year_month, country, assessment_type
           WITH NO DATA;
    SQL
