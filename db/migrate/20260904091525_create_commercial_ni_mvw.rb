class CreateCommercialNiMvw < ActiveRecord::Migration[8.1]
  def self.sql
    <<~SQL
      SELECT
  ...
  def self.up
    execute "DROP MATERIALIZED VIEW IF EXISTS mvw_commercial_ni_search"
    execute "CREATE MATERIALIZED VIEW mvw_commercial_ni_search AS
    #{sql} WITH NO DATA;"

    execute "DROP VIEW IF EXISTS vw_commercial_ni_yesterday"
    execute "CREATE VIEW vw_commercial_ni_yesterday AS
