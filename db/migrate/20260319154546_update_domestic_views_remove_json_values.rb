class UpdateDomesticViewsRemoveJsonValues < ActiveRecord::Migration[8.1]
  def self.sql
    <<~SQL
      SELECT
  ...
  def self.up
    ...
    #{sql} WHERE s.created_at::date = (CURRENT_DATE - 1) OR EXISTS(SELECT * FROM audit_logs l WHERE s.assessment_id=l.assessment_id AND event_type = 'address_id_updated' AND timestamp= (CURRENT_DATE - 1))"
  end
