class FixNonDomesticAndDisplayYesterdayViewsAuditLogFilter < ActiveRecord::Migration[8.1]
  def self.non_domestic_sql
    <<~SQL
      SELECT
  ...
  def self.up
    ...
    #{non_domestic_sql} WHERE s.created_at::date = (CURRENT_DATE - 1) OR EXISTS(SELECT * FROM audit_logs l WHERE s.assessment_id=l.assessment_id AND event_type = 'address_id_updated' AND timestamp::date = (CURRENT_DATE - 1))"

    execute "DROP VIEW IF EXISTS vw_dec_yesterday"
    execute "CREATE VIEW vw_dec_yesterday AS
    #{display_sql} WHERE s.created_at::date = (CURRENT_DATE - 1) OR EXISTS(SELECT * FROM audit_logs l WHERE s.assessment_id=l.assessment_id AND event_type = 'address_id_updated' AND timestamp::date = (CURRENT_DATE - 1))"
  end
