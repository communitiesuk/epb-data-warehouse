class VwCommercialRrYesterday < ActiveRecord::Migration[7.0]
  def self.sql
    <<~SQL
        WITH cte AS(
      ...
      JOIN assessment_documents ad ON cte.assessment_id = ad.assessment_id
    SQL
  end
  ...
  def self.up
    ...
    execute "CREATE VIEW vw_commercial_rr_yesterday AS
      #{sql} WHERE ad.warehouse_created_at::date = CURRENT_DATE - 1 OR ad.updated_at::date = (CURRENT_DATE - 1);"
  end
