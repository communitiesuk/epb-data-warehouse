...
;(async () => {
  ...
  console.log(`Starting to delete version ${versionMonth} of the ONS UPRN directory from this database`)
  try {
    ...
    const deleteNameTableSql = 'DELETE FROM ons_uprn_directory_names WHERE version_id=(SELECT id FROM ons_uprn_directory_versions WHERE version_month= $1 )'
    ...
    await db.query(deleteNameTableSql, [versionMonth])
