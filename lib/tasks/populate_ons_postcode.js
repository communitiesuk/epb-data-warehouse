...
try {
  ...
  if (targetEntries.length === 0) {
    console.error('No ONS Postcode Directory files to read in.')
    process.exit(1)
  }
...
async function insertDirectoryData (data, db, cs) {
  try {
    const dataToInsert = data.map(lowercaseObjectKeys).map(row => {
      const { pcds, ctry26cd, rgn26cd, lad26cd, pcon24cd, ...rowRest } = row
      return {
        postcode: pcds.trim(),
        country_code: ctry26cd,
        region_code: rgn26cd,
        local_authority_code: lad26cd,
        westminster_parliamentary_constituency_code: pcon24cd,
        other_areas: rowRest
      }
    })
...
function connectionOptions () {
  return {
    ...
    ssl: process.env.APP_ENV ? { rejectUnauthorized: true } : false
  }
}
