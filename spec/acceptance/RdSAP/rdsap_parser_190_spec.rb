RSpec.describe "the parser and the rdsap configuration" do
  let(:use_case) { UseCase::ParseXmlCertificate.new }

  context "when loading xml from RdSap" do
    let(:rdsap) do
      Samples.xml("RdSAP-Schema-19.0")
    end

    it "parses the document in the expected format" do
      expectation = { "schema_version_original" => "LIG-19.0",
                      "hashed_assessment_id" => "6d42518137491ecfef3053efa480079d4f1fdaf377d3be6587d0ecc713365619",
                      "sap_version" => 9.94,
                      "calculation_software_name" => "Stroma RSAP Engine",
                      "calculation_software_version" => "2.1.1.0",
                      "inspection_date" => "2020-06-03",
                      "report_type" => 2,
                      "completion_date" => "2020-06-04",
                      "registration_date" => "2020-06-04",
                      "status" => "entered",
                      "language_code" => 1,
                      "tenure" => 3,
                      "transaction_type" => 8,
                      "property_type" => 0,
                      "scheme_assessor_id" => "EXPL000008",
                      "address_line_1" => "15, Address Lane",
                      "address_line_2" => "New Town",
                      "post_town" => "Town",
                      "postcode" => "A1 1AA",
                      "uprn" => 9_999_999_999,
                      "region_code" => 19,
                      "country_code" => "EAW",
                      "walls" =>
                       [{ "description" => { "language" => "1", "value" => "Cavity wall, filled cavity" },
                          "energy_efficiency_rating" => 3,
                          "environmental_efficiency_rating" => 3 },
                        { "description" =>
                           { "language" => "1", "value" => "Cavity wall, as built, insulated (assumed)" },
                          "energy_efficiency_rating" => 4,
                          "environmental_efficiency_rating" => 4 }],
                      "roofs" =>
                       [{ "description" =>
                           { "language" => "1", "value" => "Pitched, 150 mm loft insulation" },
                          "energy_efficiency_rating" => 4,
                          "environmental_efficiency_rating" => 4 },
                        { "description" => { "language" => "1", "value" => "Pitched, insulated (assumed)" },
                          "energy_efficiency_rating" => 3,
                          "environmental_efficiency_rating" => 3 }],
                      "floors" =>
                       [{ "description" =>
                           { "language" => "1", "value" => "Solid, no insulation (assumed)" },
                          "energy_efficiency_rating" => 0,
                          "environmental_efficiency_rating" => 0 }],
                      "window" =>
                       { "description" => { "language" => "1", "value" => "Fully double glazed" },
                         "energy_efficiency_rating" => 3,
                         "environmental_efficiency_rating" => 3 },
                      "main_heating" =>
                       [{ "description" =>
                           { "language" => "1", "value" => "Boiler and radiators, mains gas" },
                          "energy_efficiency_rating" => 4,
                          "environmental_efficiency_rating" => 4 }],
                      "main_heating_controls" =>
                       [{ "description" =>
                           { "language" => "1", "value" => "Programmer, room thermostat and TRVs" },
                          "energy_efficiency_rating" => 4,
                          "environmental_efficiency_rating" => 4 }],
                      "hot_water" =>
                       { "description" => { "language" => "1", "value" => "From main system" },
                         "energy_efficiency_rating" => 4,
                         "environmental_efficiency_rating" => 4 },
                      "lighting" =>
                       { "description" =>
                          { "language" => "1", "value" => "Low energy lighting in 87% of fixed outlets" },
                         "energy_efficiency_rating" => 5,
                         "environmental_efficiency_rating" => 5 },
                      "secondary_heating" =>
                       { "description" =>
                          { "language" => "1", "value" => "Room heaters, dual fuel (mineral and wood)" },
                         "energy_efficiency_rating" => 0,
                         "environmental_efficiency_rating" => 0 },
                      "has_hot_water_cylinder" => "false",
                      "has_heated_separate_conservatory" => "false",
                      "dwelling_type" => { "language" => "1", "value" => "Semi-detached house" },
                      "total_floor_area" => 94,
                      "has_fixed_air_conditioning" => "false",
                      "multiple_glazed_proportion" => 100,
                      "energy_rating_average" => 60,
                      "energy_rating_current" => 66,
                      "energy_rating_potential" => 79,
                      "environmental_impact_current" => 62,
                      "environmental_impact_potential" => 75,
                      "energy_consumption_current" => 222,
                      "energy_consumption_potential" => 137,
                      "co2_emissions_current" => 3.8,
                      "co2_emissions_potential" => 2.4,
                      "co2_emissions_current_per_floor_area" => 41,
                      "lighting_cost_current" => { "currency" => "GBP", "value" => 81 },
                      "lighting_cost_potential" => { "currency" => "GBP", "value" => 81 },
                      "heating_cost_current" => { "currency" => "GBP", "value" => 666 },
                      "heating_cost_potential" => { "currency" => "GBP", "value" => 615 },
                      "hot_water_cost_current" => { "currency" => "GBP", "value" => 107 },
                      "hot_water_cost_potential" => { "currency" => "GBP", "value" => 74 },
                      "suggested_improvements" =>
                       [{ "sequence" => 1,
                          "improvement_category" => 5,
                          "improvement_type" => "W2",
                          "improvement_details" => { "improvement_number" => 58 },
                          "typical_saving" => { "currency" => "GBP", "value" => 51 },
                          "indicative_cost" => "£4,000 - £6,000",
                          "energy_performance_rating" => 68,
                          "environmental_impact_rating" => 65 },
                        { "sequence" => 2,
                          "improvement_category" => 5,
                          "improvement_type" => "N",
                          "improvement_details" => { "improvement_number" => 19 },
                          "typical_saving" => { "currency" => "GBP", "value" => 33 },
                          "indicative_cost" => "£4,000 - £6,000",
                          "energy_performance_rating" => 70,
                          "environmental_impact_rating" => 67 },
                        { "sequence" => 3,
                          "improvement_category" => 5,
                          "improvement_type" => "U",
                          "improvement_details" => { "improvement_number" => 34 },
                          "typical_saving" => { "currency" => "GBP", "value" => 316 },
                          "indicative_cost" => "£3,500 - £5,500",
                          "energy_performance_rating" => 79,
                          "environmental_impact_rating" => 75 }],
                      "renewable_heat_incentive" =>
                       { "space_heating_existing_dwelling" => 9825,
                         "impact_of_loft_insulation" => -394,
                         "water_heating" => 2207 },
                      "lzc_energy_sources" => [11, 9],
                      "built_form" => 2,
                      "extensions_count" => 1,
                      "multiple_glazing_type" => 3,
                      "pvc_window_frames" => "false",
                      "glazed_area" => 1,
                      "door_count" => 1,
                      "insulated_door_count" => 0,
                      "windows_transmission_details" =>
                       { "u_value" => 3.1, "solar_transmittance" => 0.76, "data_source" => 2 },
                      "percent_draughtproofed" => 100,
                      "habitable_room_count" => 5,
                      "heated_room_count" => 5,
                      "fixed_lighting_outlets_count" => 15,
                      "low_energy_fixed_lighting_outlets_count" => 13,
                      "low_energy_lighting" => 87,
                      "measurement_type" => 1,
                      "mechanical_ventilation" => 0,
                      "open_fireplaces_count" => 0,
                      "solar_water_heating" => "N",
                      "conservatory_type" => 2,
                      "sap_building_parts" =>
                       [{ "building_part_number" => 1,
                          "identifier" => "Main Dwelling",
                          "construction_age_band" => "D",
                          "sap_floor_dimensions" =>
                           [{ "floor" => 0,
                              "floor_construction" => 1,
                              "floor_insulation" => 1,
                              "heat_loss_perimeter" => { "quantity" => "metres", "value" => 13.65 },
                              "party_wall_length" => { "quantity" => "metres", "value" => 8.81 },
                              "total_floor_area" => { "quantity" => "square metres", "value" => 39.91 },
                              "room_height" => { "quantity" => "metres", "value" => 2.47 } },
                            { "floor" => 1,
                              "heat_loss_perimeter" => { "quantity" => "metres", "value" => 17.87 },
                              "party_wall_length" => { "quantity" => "metres", "value" => 8.81 },
                              "total_floor_area" => { "quantity" => "square metres", "value" => 39.91 },
                              "room_height" => { "quantity" => "metres", "value" => 2.36 } }],
                          "floor_insulation_thickness" => "NI",
                          "floor_heat_loss" => 7,
                          "roof_construction" => 4,
                          "roof_insulation_location" => 2,
                          "roof_insulation_thickness" => "150mm",
                          "wall_construction" => 4,
                          "wall_insulation_type" => 2,
                          "wall_thickness_measured" => "Y",
                          "wall_thickness" => 300,
                          "wall_dry_lined" => "N",
                          "wall_insulation_thickness" => "NI",
                          "party_wall_construction" => 1 },
                        { "building_part_number" => 2,
                          "identifier" => "Extension",
                          "construction_age_band" => "G",
                          "sap_floor_dimensions" =>
                           [{ "floor" => 0,
                              "floor_construction" => 1,
                              "floor_insulation" => 1,
                              "heat_loss_perimeter" => { "quantity" => "metres", "value" => 10.8 },
                              "party_wall_length" => 0,
                              "total_floor_area" => { "quantity" => "square metres", "value" => 13.88 },
                              "room_height" => { "quantity" => "metres", "value" => 2.34 } }],
                          "floor_insulation_thickness" => "NI",
                          "floor_heat_loss" => 7,
                          "roof_construction" => 5,
                          "roof_insulation_location" => 4,
                          "roof_insulation_thickness" => "ND",
                          "wall_construction" => 4,
                          "wall_insulation_type" => 4,
                          "wall_thickness_measured" => "Y",
                          "wall_thickness" => 300,
                          "wall_dry_lined" => "N",
                          "wall_insulation_thickness" => "NI",
                          "party_wall_construction" => "NA" }],
                      "sap_heating" =>
                       { "main_heating_details" =>
                          [{ "main_heating_number" => 1,
                             "main_heating_category" => 2,
                             "main_fuel_type" => 26,
                             "main_heating_control" => 2106,
                             "main_heating_data_source" => 1,
                             "main_heating_index_number" => 15_274,
                             "boiler_flue_type" => 2,
                             "heat_emitter_type" => 1,
                             "main_heating_fraction" => 1,
                             "has_fghrs" => "N",
                             "emitter_temperature" => 0,
                             "central_heating_pump_age" => 0 }],
                         "water_heating_code" => 901,
                         "water_heating_fuel" => 26,
                         "secondary_heating_type" => 631,
                         "secondary_fuel_type" => 9,
                         "immersion_heating_type" => "NA",
                         "cylinder_size" => 1,
                         "has_fixed_air_conditioning" => "false",
                         "instantaneous_wwhrs" =>
                          { "rooms_with_bath_and_or_shower" => 1,
                            "rooms_with_mixer_shower_no_bath" => 0,
                            "rooms_with_bath_and_mixer_shower" => 0 } },
                      "sap_energy_source" =>
                       { "meter_type" => 3,
                         "mains_gas" => "Y",
                         "wind_turbines_count" => 0,
                         "wind_turbines_terrain_type" => 2,
                         "photovoltaic_supply" =>
                          { "none_or_no_details" => { "percent_roof_area" => 0, "pv_connection" => 0 } } } }

      expect(use_case.execute(xml: rdsap,
                              schema_type: "RdSAP-Schema-19.0",
                              assessment_id: "0000-6666-4444-3333-2222")).to eq(expectation)
    end
  end
end
