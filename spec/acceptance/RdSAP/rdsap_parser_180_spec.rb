RSpec.describe "the parser and the RdSAP configuration for 18.0" do
  let(:use_case) { UseCase::ParseXmlCertificate.new }

  context "when loading XML from RdSAP" do
    let(:rdsap) { Samples.xml "RdSAP-Schema-18.0" }

    it "parses the document in the expected format" do
      expectation = { "schema_version_original" => "LIG-17.0",
                      "sap_version" => 9.92,
                      "calculation_software_name" => "Stroma RSAP Engine",
                      "calculation_software_version" => "2.0.0.0",
                      "inspection_date" => "2017-03-19",
                      "report_type" => 2,
                      "completion_date" => "2017-03-19",
                      "registration_date" => "2017-03-19",
                      "status" => "entered",
                      "language_code" => 1,
                      "tenure" => 1,
                      "transaction_type" => 5,
                      "property_type" => 0,
                      "scheme_assessor_id" => "SCHE000011",
                      "address_line_1" => "1, Bagshot Lane",
                      "address_line_2" => "Village",
                      "post_town" => "POSTTOWN",
                      "postcode" => "PT11 4RF",
                      "uprn" => 22_223_333,
                      "region_code" => 1,
                      "country_code" => "EAW",
                      "walls" =>
                       [{ "description" => { "language" => "1", "value" => "Solid brick, as built, no insulation (assumed)" },
                          "energy_efficiency_rating" => 1,
                          "environmental_efficiency_rating" => 1 },
                        { "description" => { "language" => "1", "value" => "Cavity wall, as built, insulated (assumed)" },
                          "energy_efficiency_rating" => 4,
                          "environmental_efficiency_rating" => 4 }],
                      "roofs" =>
                       [{ "description" => { "language" => "1", "value" => "Pitched, 100 mm loft insulation" }, "energy_efficiency_rating" => 3, "environmental_efficiency_rating" => 3 },
                        { "description" => { "language" => "1", "value" => "Flat, insulated" }, "energy_efficiency_rating" => 3, "environmental_efficiency_rating" => 3 },
                        { "description" => { "language" => "1", "value" => "Roof room(s), insulated (assumed)" }, "energy_efficiency_rating" => 4, "environmental_efficiency_rating" => 4 }],
                      "floors" => [{ "description" => { "language" => "1", "value" => "Solid, no insulation (assumed)" }, "energy_efficiency_rating" => 0, "environmental_efficiency_rating" => 0 }],
                      "window" => { "description" => { "language" => "1", "value" => "Fully double glazed" }, "energy_efficiency_rating" => 3, "environmental_efficiency_rating" => 3 },
                      "main-heating" =>
                       [{ "description" => { "language" => "1", "value" => "Boiler and radiators, mains gas" }, "energy_efficiency_rating" => 4, "environmental_efficiency_rating" => 4 }],
                      "main_heating_controls" =>
                       [{ "description" => { "language" => "1", "value" => "Programmer, room thermostat and TRVs" }, "energy_efficiency_rating" => 4, "environmental_efficiency_rating" => 4 }],
                      "hot_water" => { "description" => { "language" => "1", "value" => "From main system" }, "energy_efficiency_rating" => 4, "environmental_efficiency_rating" => 4 },
                      "lighting" =>
                       { "description" => { "language" => "1", "value" => "Low energy lighting in 67% of fixed outlets" },
                         "energy_efficiency_rating" => 4,
                         "environmental_efficiency_rating" => 4 },
                      "secondary_heating" => { "description" => { "language" => "1", "value" => "None" }, "energy_efficiency_rating" => 0, "environmental_efficiency_rating" => 0 },
                      "has_hot_water_cylinder" => "false",
                      "has_heated_separate_conservatory" => "false",
                      "dwelling_type" => { "language" => "1", "value" => "Mid-terrace house" },
                      "total_floor_area" => 93,
                      "has_fixed_air_conditioning" => "false",
                      "multiple_glazed_proportion" => 100,
                      "energy_rating_average" => 60,
                      "energy_rating_current" => 69,
                      "energy_rating_potential" => 85,
                      "environmental_impact_current" => 66,
                      "environmental_impact_potential" => 83,
                      "energy_consumption_current" => 230,
                      "energy_consumption_potential" => 115,
                      "co2_emissions_current" => 3.4,
                      "co2_emissions_potential" => 1.7,
                      "co2_emissions_current_per_floor_area" => 41,
                      "lighting_cost_current" => { "currency" => "GBP", "value" => 81 },
                      "lighting_cost_potential" => { "currency" => "GBP", "value" => 61 },
                      "heating_cost_current" => { "currency" => "GBP", "value" => 619 },
                      "heating_cost_potential" => { "currency" => "GBP", "value" => 534 },
                      "hot_water_cost_current" => { "currency" => "GBP", "value" => 100 },
                      "hot_water_cost_potential" => { "currency" => "GBP", "value" => 68 },
                      "suggested_improvements" =>
                       [{ "sequence" => 1,
                          "improvement_category" => 5,
                          "improvement_type" => "Q",
                          "improvement_details" => { "improvement_number" => 7 },
                          "typical_saving" => { "currency" => "GBP", "value" => 88 },
                          "indicative_cost" => "£4,000 - £14,000",
                          "energy_performance_rating" => 72,
                          "environmental_impact_rating" => 71 },
                        { "sequence" => 2,
                          "improvement_category" => 5,
                          "improvement_type" => "E",
                          "improvement_details" => { "improvement_number" => 35 },
                          "typical_saving" => { "currency" => "GBP", "value" => 18 },
                          "indicative_cost" => "£15",
                          "energy_performance_rating" => 73,
                          "environmental_impact_rating" => 71 },
                        { "sequence" => 3,
                          "improvement_category" => 5,
                          "improvement_type" => "N",
                          "improvement_details" => { "improvement_number" => 19 },
                          "typical_saving" => { "currency" => "GBP", "value" => 33 },
                          "indicative_cost" => "£4,000 - £6,000",
                          "energy_performance_rating" => 74,
                          "environmental_impact_rating" => 73 },
                        { "sequence" => 4,
                          "improvement_category" => 5,
                          "improvement_type" => "U",
                          "improvement_details" => { "improvement_number" => 34 },
                          "typical_saving" => { "currency" => "GBP", "value" => 302 },
                          "indicative_cost" => "£5,000 - £8,000",
                          "energy_performance_rating" => 85,
                          "environmental_impact_rating" => 83 }],
                      "renewable_heat_incentive" =>
                       { "space_heating_existing_dwelling" => 10_483,
                         "impact_of_loft_insulation" => -214,
                         "impact_of_solid_wall_insulation" => -1864,
                         "water_heating" => 2087 },
                      "lzc_energy_sources" => [11],
                      "built_form" => 4,
                      "extensions_count" => 1,
                      "multiple_glazing_type" => 3,
                      "pvc_window_frames" => "true",
                      "glazing_gap" => 12,
                      "glazed_area" => 1,
                      "door_count" => 2,
                      "insulated_door_count" => 0,
                      "percent_draughtproofed" => 100,
                      "habitable_room_count" => 5,
                      "heated_room_count" => 5,
                      "fixed_lighting_outlets_count" => 9,
                      "low_energy_fixed_lighting_outlets_count" => 6,
                      "low_energy_lighting" => 67,
                      "measurement_type" => 1,
                      "mechanical_ventilation" => 0,
                      "open_fireplaces_count" => 0,
                      "solar_water_heating" => "N",
                      "conservatory_type" => 1,
                      "sap_building_parts" =>
                       [{ "building_part_number" => 1,
                          "identifier" => "Main Dwelling",
                          "construction_age_band" => "C",
                          "sap_floor_dimensions" =>
                           [{ "floor" => 0,
                              "floor_construction" => 1,
                              "floor_insulation" => 1,
                              "heat_loss_perimeter" => { "quantity" => "metres", "value" => 5.2 },
                              "party_wall_length" => { "quantity" => "metres", "value" => 11.2 },
                              "total_floor_area" => { "quantity" => "square metres", "value" => 29.12 },
                              "room_height" => { "quantity" => "metres", "value" => 2.4 } },
                            { "floor" => 1,
                              "heat_loss_perimeter" => { "quantity" => "metres", "value" => 10.4 },
                              "party_wall_length" => { "quantity" => "metres", "value" => 11.2 },
                              "total_floor_area" => { "quantity" => "square metres", "value" => 29.12 },
                              "room_height" => { "quantity" => "metres", "value" => 2.4 } }],
                          "floor_heat_loss" => 7,
                          "roof_construction" => 4,
                          "roof_insulation_location" => 2,
                          "roof_insulation_thickness" => "100mm",
                          "wall_construction" => 3,
                          "wall_insulation_type" => 4,
                          "wall_thickness_measured" => "Y",
                          "wall_thickness" => 330,
                          "wall_dry_lined" => "N",
                          "wall_insulation_thickness" => "NI",
                          "party_wall_construction" => 0,
                          "sap_room_in_roof" => { "floor_area" => { "quantity" => "square metres", "value" => 9 }, "construction_age_band" => "G", "insulation" => "AB", "roof_room_connected" => "N" } },
                        { "building_part_number" => 2,
                          "identifier" => "Extension",
                          "construction_age_band" => "G",
                          "sap_floor_dimensions" =>
                           [{ "floor" => 0,
                              "floor_construction" => 1,
                              "floor_insulation" => 1,
                              "heat_loss_perimeter" => { "quantity" => "metres", "value" => 5.2 },
                              "party_wall_length" => { "quantity" => "metres", "value" => 6 },
                              "total_floor_area" => { "quantity" => "square metres", "value" => 15.6 },
                              "room_height" => { "quantity" => "metres", "value" => 2.4 } }],
                          "floor_heat_loss" => 7,
                          "roof_construction" => 1,
                          "roof_insulation_location" => 6,
                          "flat_roof_insulation_thickness" => "NI",
                          "wall_construction" => 4,
                          "wall_insulation_type" => 4,
                          "wall_thickness_measured" => "Y",
                          "wall_thickness" => 290,
                          "wall_dry_lined" => "N",
                          "wall_insulation_thickness" => "NI",
                          "party_wall_construction" => 0 }],
                      "sap_heating" =>
                       { "main_heating_details" =>
                          [{ "main_heating_number" => 1,
                             "main_heating_category" => 2,
                             "main_fuel_type" => 26,
                             "main_heating_control" => 2106,
                             "main_heating_data_source" => 1,
                             "main_heating_index_number" => 16_137,
                             "boiler_flue_type" => 2,
                             "heat_emitter_type" => 1,
                             "main_heating_fraction" => 1,
                             "has_fghrs" => "N",
                             "emitter_temperature" => 0,
                             "central_heating_pump_age" => 0 }],
                         "water_heating_code" => 901,
                         "water_heating_fuel" => 26,
                         "immersion_heating_type" => "NA",
                         "cylinder_size" => 1,
                         "has_fixed_air_conditioning" => "false",
                         "instantaneous_wwhrs" =>
                           { "rooms_with_bath_and_or_shower" => 1,
                             "rooms_with_mixer_shower_no_bath" => 0,
                             "rooms_with_bath_and_mixer_shower" => 0 } },
                      "sap_energy_source" =>
                       { "meter_type" => 1,
                         "mains_gas" => "Y",
                         "wind_turbines_count" => 0,
                         "wind_turbines_terrain_type" => 2,
                         "photovoltaic_supply" =>
                           { "none_or_no_details" => { "percent_roof_area" => 0, "pv_connection" => 0 } } } }

      expect(use_case.execute(xml: rdsap,
                              schema_type: "RdSAP-Schema-18.0",
                              assessment_id: "0000-1111-2222-3333-4444")).to eq(expectation)
    end
  end
end
