RSpec.describe "parsing with an NI RdSAP configuration (17.3)" do
  let(:use_case) { UseCase::ParseXmlCertificate.new }

  context "when loading XML from RdSAP (NI)" do
    let(:rdsap) do
      Samples.xml("RdSAP-Schema-NI-17.3")
    end

    it "parses the document in the expected format" do
      expectation = { "schema_version_original" => "LIG-17.3",
                      "hashed_assessment_id" => "60cbb6dbf6f2702d860cfb18fcefa26af5a3a7fe618c5778087b9ad8cce57b4a",
                      "sap_version" => 9.92,
                      "calculation_software_name" => "Elmhurst Energy Systems RdSAP Calculator",
                      "calculation_software_version" => "2.04r06",
                      "inspection_date" => "2015-08-11",
                      "report_type" => 2,
                      "completion_date" => "2015-08-11",
                      "registration_date" => "2015-08-11",
                      "status" => "entered",
                      "language_code" => 1,
                      "tenure" => 1,
                      "transaction_type" => 1,
                      "property_type" => 0,
                      "scheme_assessor_id" => "SCHE059555",
                      "address_line_1" => "7, Virtual Lane",
                      "post_town" => "BELFAST",
                      "postcode" => "BT1 1AA",
                      "uprn" => 8_768_769_876,
                      "region_code" => 10,
                      "country_code" => "NIR",
                      "walls" =>
                       [{ "description" => "Cavity wall, as built, insulated (assumed)",
                          "energy_efficiency_rating" => 4,
                          "environmental_efficiency_rating" => 4 }],
                      "roofs" =>
                       [{ "description" => "Pitched, insulated (assumed)",
                          "energy_efficiency_rating" => 4,
                          "environmental_efficiency_rating" => 4 }],
                      "floors" =>
                       [{ "description" => "Solid, insulated (assumed)",
                          "energy_efficiency_rating" => 0,
                          "environmental_efficiency_rating" => 0 }],
                      "window" =>
                       { "description" => "Fully double glazed",
                         "energy_efficiency_rating" => 4,
                         "environmental_efficiency_rating" => 4 },
                      "main_heating" =>
                       [{ "description" => "Boiler and radiators, oil",
                          "energy_efficiency_rating" => 3,
                          "environmental_efficiency_rating" => 3 }],
                      "main_heating_controls" =>
                       [{ "description" => "Programmer and at least two room thermostats",
                          "energy_efficiency_rating" => 4,
                          "environmental_efficiency_rating" => 4 }],
                      "hot_water" =>
                       { "description" => "From main system, no cylinder thermostat",
                         "energy_efficiency_rating" => 2,
                         "environmental_efficiency_rating" => 2 },
                      "lighting" =>
                       { "description" => "Low energy lighting in 63% of fixed outlets",
                         "energy_efficiency_rating" => 4,
                         "environmental_efficiency_rating" => 4 },
                      "secondary_heating" =>
                       { "description" => "Room heaters, coal",
                         "energy_efficiency_rating" => 0,
                         "environmental_efficiency_rating" => 0 },
                      "has_hot_water_cylinder" => "true",
                      "has_heated_separate_conservatory" => "false",
                      "dwelling_type" => "Detached house",
                      "total_floor_area" => 175,
                      "has_fixed_air_conditioning" => "false",
                      "multiple_glazed_proportion" => 100,
                      "energy_rating_average" => 60,
                      "energy_rating_current" => 70,
                      "energy_rating_potential" => 73,
                      "environmental_impact_current" => 59,
                      "environmental_impact_potential" => 64,
                      "energy_consumption_current" => 140,
                      "energy_consumption_potential" => 125,
                      "co2_emissions_current" => 6.8,
                      "co2_emissions_potential" => 6.1,
                      "co2_emissions_current_per_floor_area" => 39,
                      "lighting_cost_current" => { "currency" => "GBP", "value" => 121 },
                      "lighting_cost_potential" => { "currency" => "GBP", "value" => 88 },
                      "heating_cost_current" => { "currency" => "GBP", "value" => 852 },
                      "heating_cost_potential" => { "currency" => "GBP", "value" => 788 },
                      "hot_water_cost_current" => { "currency" => "GBP", "value" => 208 },
                      "hot_water_cost_potential" => { "currency" => "GBP", "value" => 169 },
                      "suggested_improvements" =>
                       [{ "sequence" => 1,
                          "improvement_category" => 1,
                          "improvement_type" => "E",
                          "improvement_details" => { "improvement_number" => 35 },
                          "typical_saving" => { "currency" => "GBP", "value" => 25 },
                          "indicative_cost" => "£30",
                          "energy_performance_rating" => 70,
                          "environmental_impact_rating" => 60 },
                        { "sequence" => 2,
                          "improvement_category" => 1,
                          "improvement_type" => "F",
                          "improvement_details" => { "improvement_number" => 4 },
                          "typical_saving" => { "currency" => "GBP", "value" => 66 },
                          "indicative_cost" => "£200 - £400",
                          "energy_performance_rating" => 72,
                          "environmental_impact_rating" => 62 },
                        { "sequence" => 3,
                          "improvement_category" => 1,
                          "improvement_type" => "G",
                          "improvement_details" => { "improvement_number" => 16 },
                          "typical_saving" => { "currency" => "GBP", "value" => 46 },
                          "indicative_cost" => "£350 - £450",
                          "energy_performance_rating" => 73,
                          "environmental_impact_rating" => 64 },
                        { "sequence" => 4,
                          "improvement_category" => 3,
                          "improvement_type" => "N",
                          "improvement_details" => { "improvement_number" => 19 },
                          "typical_saving" => { "currency" => "GBP", "value" => 66 },
                          "indicative_cost" => "£4,000 - £6,000",
                          "energy_performance_rating" => 75,
                          "environmental_impact_rating" => 66 },
                        { "sequence" => 5,
                          "improvement_category" => 3,
                          "improvement_type" => "U",
                          "improvement_details" => { "improvement_number" => 34 },
                          "typical_saving" => { "currency" => "GBP", "value" => 252 },
                          "indicative_cost" => "£5,000 - £8,000",
                          "energy_performance_rating" => 81,
                          "environmental_impact_rating" => 71 }],
                      "built_form" => 1,
                      "extensions_count" => 0,
                      "multiple_glazing_type" => 2,
                      "glazed_area" => 1,
                      "door_count" => 2,
                      "insulated_door_count" => 0,
                      "percent_draughtproofed" => 100,
                      "habitable_room_count" => 7,
                      "heated_room_count" => 7,
                      "fixed_lighting_outlets_count" => 16,
                      "low_energy_fixed_lighting_outlets_count" => 10,
                      "low_energy_lighting" => 63,
                      "measurement_type" => 1,
                      "mechanical_ventilation" => 0,
                      "open_fireplaces_count" => 0,
                      "solar_water_heating" => "N",
                      "conservatory_type" => 1,
                      "lzc_energy_sources" => [11],
                      "sap_building_parts" =>
                       [{ "building_part_number" => 1,
                          "identifier" => "Main Dwelling",
                          "construction_age_band" => "K",
                          "sap_floor_dimensions" =>
                           [{ "floor" => 0,
                              "floor_construction" => 1,
                              "floor_insulation" => 1,
                              "heat_loss_perimeter" => { "quantity" => "metres", "value" => 40.16 },
                              "party_wall_length" => { "quantity" => "metres", "value" => 0 },
                              "total_floor_area" => { "quantity" => "square metres", "value" => 91.89 },
                              "room_height" => { "quantity" => "metres", "value" => 2.4 } },
                            { "floor" => 1,
                              "heat_loss_perimeter" => { "quantity" => "metres", "value" => 40 },
                              "party_wall_length" => { "quantity" => "metres", "value" => 0 },
                              "total_floor_area" => { "quantity" => "square metres", "value" => 83.28 },
                              "room_height" => { "quantity" => "metres", "value" => 2.42 } }],
                          "floor_heat_loss" => 7,
                          "roof_construction" => 5,
                          "roof_insulation_location" => 4,
                          "roof_insulation_thickness" => "ND",
                          "wall_construction" => 4,
                          "wall_insulation_type" => 4,
                          "wall_thickness_measured" => "N",
                          "wall_dry_lined" => "N",
                          "wall_insulation_thickness" => "NI",
                          "party_wall_construction" => "NA" }],
                      "sap_heating" =>
                       { "main_heating_details" =>
                          [{ "main_heating_number" => 1,
                             "main_heating_category" => 2,
                             "main_fuel_type" => 28,
                             "main_heating_control" => 2105,
                             "main_heating_data_source" => 2,
                             "sap_main_heating_code" => 127,
                             "boiler_flue_type" => 2,
                             "fan_flue_present" => "Y",
                             "heat_emitter_type" => 1,
                             "main_heating_fraction" => 1,
                             "has_fghrs" => "N",
                             "emitter_temperature" => 0,
                             "central_heating_pump_age" => 1 }],
                         "water_heating_code" => 901,
                         "water_heating_fuel" => 28,
                         "secondary_heating_type" => 631,
                         "secondary_fuel_type" => 33,
                         "immersion_heating_type" => "NA",
                         "cylinder_size" => 3,
                         "cylinder_insulation_type" => 1,
                         "cylinder_insulation_thickness" => 50,
                         "cylinder_thermostat" => "N",
                         "has_fixed_air_conditioning" => "false",
                         "instantaneous_wwhrs" =>
                          { "rooms_with_bath_and_or_shower" => 2,
                            "rooms_with_mixer_shower_no_bath" => 0,
                            "rooms_with_bath_and_mixer_shower" => 1 } },
                      "sap_energy_source" =>
                       { "meter_type" => 2,
                         "mains_gas" => "N",
                         "wind_turbines_count" => 0,
                         "wind_turbines_terrain_type" => 2,
                         "photovoltaic_supply" =>
                          { "none_or_no_details" =>
                             { "percent_roof_area" => 0,
                               "pv_connection" => 0 } } } }

      expect(use_case.execute(xml: rdsap,
                              schema_type: "RdSAP-Schema-NI-17.3",
                              assessment_id: "0000-2222-4444-6666-8888")).to eq(expectation)
    end
  end
end
