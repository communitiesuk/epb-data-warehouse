describe "the parser and the CEPC configuration for schema 5.1" do
  let(:use_case) { UseCase::ParseXmlCertificate.new }

  context "when loading XML from CEPC" do
    let(:cepc) { Samples.xml "CEPC-5.1", "cepc" }
  ...
  context "when loading XML from CEPC-RR" do
    let(:cepc_rr) { Samples.xml "CEPC-5.1", "cepc+rr" }
    ...
    it "parses the document in the expected format" do
      expectation = { "address_line_1" => "Some Unit",
                      ...
                      "related_rrn" => "4444-0000-0000-0000-0000",
                      "report_type" => 4,
                      "scheme_assessor_id" => "SPEC000000",
      ...
      actual = use_case.execute(xml: cepc_rr,
                                schema_type: "CEPC-5.1",
                                assessment_id: "4444-0000-0000-0000-0001")
      expect(actual).to eq(expectation)
    end
  end

  context "when loading XML from DEC" do
    let(:dec) { Samples.xml "CEPC-5.1", "dec" }
  ...
  context "when loading XML from DEC-RR" do
    let(:dec_rr) { Samples.xml "CEPC-5.1", "dec-rr" }
