describe "the parser and the CEPC configuration for schema 5.0" do
  let(:use_case) { UseCase::ParseXmlCertificate.new }

  context "when loading XML from CEPC" do
    let(:cepc) { Samples.xml "CEPC-5.0", "cepc" }

    it "parses the document in the expected format" do
      expectation = { "ac_questionnaire" => { "ac_estimated_output" => 3, "ac_inspection_commissioned" => 1, "ac_present" => "No", "ac_rated_output" => { "ac_kw_rating" => 100 } },
                      ...
                      "or_benchmark_data" => { "benchmark_1" => { "floor_area" => 10 } },
                      "or_building_data" => { "assessment_period_alignment" => "Yes", "hvac_system" => "Yes", "internal_environment" => "Yes", "other_hvac_system" => "Yes" },
                      ...
                      "renewable_energy_source" => [{ "end_date" => "2030-12-25", "energy_type" => 2, "generation" => 20, "name" => "Solar panel", "start_date" => "2019-12-25" }],
                      "report_type" => 3,
  ...
  context "when loading XML from CEPC-RR" do
    let(:cepc_rr) { Samples.xml "CEPC-5.0", "cepc+rr" }
    ...
    it "parses the document in the expected format" do
      expectation = { "address_line_1" => "Some Unit",
                      ...
                      "related_rrn" => "3333-0000-0000-0000-0000",
                      "report_type" => 4,
                      "scheme_assessor_id" => "SPEC000000",
      ...
      actual = use_case.execute(xml: cepc_rr,
                                schema_type: "CEPC-5.0",
                                assessment_id: "3333-0000-0000-0000-0001")
      expect(actual).to eq(expectation)
    end
  end

  context "when loading XML from DEC" do
    let(:dec) { Samples.xml "CEPC-5.0", "dec" }
  ...
  context "when loading XML from DEC-RR" do
    let(:dec_rr) { Samples.xml "CEPC-5.0", "dec-rr" }
