  ...
  context "when creating commercial materialized view" do
    ...
    context "when an assessment has a URPN value in the assessment_address_id attribute" do
      it "returns a value for the uprn_source" do
        results = gateway.fetch(**search_arguments)
        expect(results.find { |i| i["certificate_number"] == "0000-0000-0000-0000-0009" }["uprn_source"]).to eq "Energy Assessor"
      end
    end
    ...
    context "when checking the columns of the materialized view" do
      ...
      it "returns the correct columns" do
        expect(gateway.columns.sort.map(&:downcase)).to eq expected_columns.sort.map(&:downcase)
      end
    end
