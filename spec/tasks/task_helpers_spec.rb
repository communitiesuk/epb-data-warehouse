describe Tasks::TaskHelpers do
  ...
  describe "#get_last_months_methods" do
    context "when it is the first day of the month (1st March 2022)" do
      before do
        Timecop.freeze(2022, 3, 1, 7, 0, 0)
      end
      ...
      it "returns the 1st day of the previous month" do
        expect(described_class.get_last_months_dates[:start_date]).to eq "2022-02-01"
      end

      it "returns the last day of the previous month" do
        expect(described_class.get_last_months_dates[:end_date]).to eq "2022-02-28"
      end
    end
  end
end
